...
const CPlayerDesc CAdPlug::allplayers[] = {
  ...
  CPlayerDesc(CamdLoader::factory, "AMUSIC Adlib Tracker", ".amd\0"),
...
CPlayer *CAdPlug::factory(const std::string &fn, Copl *opl, char*& type, const CPlayers &pl,
  ...
  for(i = pl.begin(); i != pl.end(); i++)
	...
	if((p = (*i)->factory(opl))) {
	  if(p->load(fn, fp)) {
		...
		type = const_cast<char*>((*i)->filetype.c_str());
	    return p;
	  } else
	    delete p;
	}
